...
> **Capability level: v0 (file-driven).** State detection is based on files under
> `docs/brainstorm/` only. There is no `control.json`, no turn lock, no daemon, no atomic
> abort and no structured checkpoint yet — those arrive with REQ-004's full implementation.
> Do not claim capabilities beyond this file.

## 1. Detect state (never guess)

- Open round = a `docs/brainstorm/rodadas/RODADA-NNN/` directory WITHOUT `DECISAO.md`.
...
- **`brainstorm "<question>"` + nothing open** → create the next `RODADA-NNN/` with
...
## 3. Drive the round (protocol in `docs/brainstorm/REGRAS.md`)
...
5. `DECISAO.md` — what was decided, who closed it, what was executed. Then execute it
   (PRD update, etc.) and commit ONLY regent-owned paths (`docs/`, `.regent/`, `.claude/`
   symlinks) — never unrelated host content. A failed commit does not undo the round; report
   it and leave it pending.
